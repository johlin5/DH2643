...
const QuizForm: React.FC<QuizFromProps> = ({setEdit, editState, quiz, setQuizData}: QuizFromProps) => {
  ...
  const updateQuestion = (index: number, data: QuestionInput) => {
    let newQuestions = [...questions]; // copying the old datas array
  ...
  return (
    <Container component="main" maxWidth="xs" style={{ backgroundColor: "white", padding: "16px", marginTop: "32px" }}>
      <TextField id="standard-basic" label="Standard" variant="standard" margin="normal" 
        ...
        <PrimaryButton text="Add question" color={GREEN} variant="h6" height="48px" onClick={() => addQuestion({question: "", answers: [], id: "", userId: "", upvotes: 0, report:""})} />
        <PrimaryButton text="Save Quiz" color={PURPLE} variant="h6" height="48px" onClick={() => handleSaveQuiz()} />
        <PrimaryButton text="Delete Quiz" color={RED} variant="h6" height="48px" onClick={() => {/** Add function that handles deletion */}} />
    </Container>
  );
};
