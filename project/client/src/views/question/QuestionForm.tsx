...
const QuestionForm: React.FC<QuestionFormProps> = ({saveQuestion, editQuiz, data} : QuestionFormProps) => {
  ...
  const [editorState, setEditorState] = useState<EditorState>(() => EditorState.createWithContent(ContentState.createFromText(data.question)));
  ...
  const updateAnswer = (index: number, data: AnswerInput) => {
    let newAnswers = [...answers]; // copying the old datas array
  ...
  return (
    <Container component="main" maxWidth="xs" style={{ backgroundColor: "white", padding: "16px", marginTop: "32px" }}>
      ...
      <ul>
        ...
        <PrimaryButton text="Add Answer" color={TURQUOISE} variant="h6" height="48px" onClick={() => addAnswer({description: "", id: "", flag: "0"})}/>
        <PrimaryButton text="Save" color={PURPLE} variant="h6" height="48px" onClick={() => handleSaveQuestion(editorState)}/>
        <PrimaryButton text="-" color={RED} variant="h6" height="48px" onClick={() => {/** Add function that handles deletion */}} />
    </Container>
  );
};
